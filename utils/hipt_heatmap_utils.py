...
from utils.attention_visualization_utils import get_patch_attention_scores, tensorbatch2im, concat_scores256
...
def create_patch_heatmaps_concat(patch, model256, output_dir, fname, threshold=0.5,
    ...
    if threshold != None:
        ...
        getConcatImage([getConcatImage(ths[0:3]),
                        getConcatImage(ths[3:6])], how='vertical').save(
            os.path.join(output_dir, '%s_256th.png' % (fname)))
...
def get_region_attention_scores(region, model256, model4k, scale=1,
    ...
    with torch.no_grad():
        ...
        attention_4096 = \
        nn.functional.interpolate(attention_4096.unsqueeze(0), scale_factor=int(256 / scale), mode="nearest")[
            0].cpu().numpy()
...
def create_hierarchical_heatmaps_indiv(region, model256, model4k, output_dir, fname,
    ...
    b256_1, a256_1, a4k_1 = get_region_attention_scores(region, model256, model4k, scale, device256=device256,
                                                        device4k=device4k)
    b256_2, a256_2, a4k_2 = get_region_attention_scores(region2, model256, model4k, scale, device256=device256,
                                                        device4k=device4k)
    b256_3, a256_3, a4k_3 = get_region_attention_scores(region3, model256, model4k, scale, device256=device256,
                                                        device4k=device4k)
    b256_4, a256_4, a4k_4 = get_region_attention_scores(region4, model256, model4k, scale, device256=device256,
                                                        device4k=device4k)
    ...
    for j in range(6):
        ...
        for i in range(6):
            ...
            Image.fromarray(region_hm).save(
                os.path.join(output_dir, '%s_factorized_4k[%s]_256[%s].png' % (fname, j, i)))
